...
- Exercise 4.10: source and Kubernetes desired state are now split between `devops-with-kubernetes-2026` and `devops-with-kubernetes-2026-config`. The config repository validates both staging and production Kustomize overlays successfully. End-to-end cross-repository image update still requires the source repository Actions secret `CONFIG_REPO_TOKEN` with Contents write permission on the config repository.
